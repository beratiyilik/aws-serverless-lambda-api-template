...
const customStringify = (
  ...
  if (Array.isArray(value))
    return serializeArray(value, replacer, space, isCyclic, comparer, level);
  else
...
const serializeArray = (
  ...
  const items = arr.map(
    (item) =>
      customStringify(item, replacer, space, isCyclic, comparer, level + 1) ||
      JSON.stringify(null)
  );
  const indent = getIndent(space, level);
...
const serializeObject = (
  ...
  const items = keys
    .map((key) => {
      ...
      return value
        ? `${JSON.stringify(key)}${colonSeparator}${value}`
        : undefined;
    })
    .filter(Boolean);
  return `{${items.join(",")}${indent}}`;
...
const toJSON = (value: any, options?: SerializationOptions): string =>
  jsonStableStringify(value, { ...options, space: 2 });
const toObject = <Type = unknown>(
  text: string,
